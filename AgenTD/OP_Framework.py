...
def log_target_network_full(target_dat_name='OP_Framework'):
	...
	def is_non_default_par(p):
		...
		try:
			...
			return not bool(is_default)
		except:
			pass

		try:
			default_val = p.default
			return str(p.val) != str(default_val)
		except:
			pass
	...
	def read_table_dat_content(dat_op):
		...
		hash_src = json.dumps(rows, ensure_ascii=False)
		return {
			'kind': 'table',
			'meta': {
				'rows': row_count,
	...
	try:
		payload = json.dumps(forest, indent='\t', ensure_ascii=False)
		target_dat.text = payload
		export_path = get_export_path()
		with open(export_path, 'w', encoding='utf-8') as f:
			f.write(payload)
		print(f"Success! Scanned {len(forest)} top-level nodes in /project1 -> {export_path}")
